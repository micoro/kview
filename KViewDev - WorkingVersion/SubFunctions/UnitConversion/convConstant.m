    ...
    methods
        ...
        function y=define(obj,x)
            ...
            switch x
                ...
                case {'MPa'}                                 % pressure in pascals
                    y=units2convFac(unit(1000000,'N/m^2'));
                ...
                % Derived Units (Torque/Moment)
                case {'Nm'}                                  % Torque in Newton*meter
                    y=units2convFac(unit('N*m')); 
                case {'Nmm'}                                 % Torque in Newton*millimeter
                    y=units2convFac(unit('N*mm'));        
                case {'kgm'}                                 % Torque in kilos*meter
                    y=units2convFac(unit(9.81,'N*m'));      
                ...
                % Energy
                case {'kWh'}                                 % energy kilowatt per hour
                    y=units2convFac(3600000*unit('J'));
                ...
                % Length
                case {'mm'}                                  % length in millimeters
                    y.unitName='mm';y.convValue=[0.001 0];y.convUnit='m';y.fundamental=false;
                ...
                case {'h','hour','hours','hr'}                   % time in hours
                ...
                % Temperature
                case {'celsius','degC','°C'}                      % temperature in degrees Celsius
                    y.unitName='degC';y.convValue=[1 273.15];y.convUnit='kelvin';y.fundamental=false;
                case {'degF','°F'}                                % temperature in degrees Fahrenheit
                ...
                % Volume
                case {'liter','liters','L','l'}               % volume in liters
                ...
                % Angular Velocity
                case {'rpm'}                                  % velocity in miles per hour
                    y=units2convFac(unit('rev/minute'));                
...
%                     y=units2convFac(unit(6.62607540e-34,'joule*sec'));
          ...
          % Other
                case {'radian','radians','rad'}              % angle in radians
                    y.unitName='rad';y.convValue=[1 0];y.convUnit='rad';y.fundamental=true;
%                     y=units2convFac(unit);  
                case {'degree','degrees','deg'}              % angle in degrees
                    y=units2convFac(pi/180*unit('rad'));
                case {'cycle','cycles','rev','round'}        % a periodic unit
                    y=units2convFac(2*pi*unit('rad')); 
                case {'hertz','Hz'}                          % frequency in hertz
                    y=units2convFac(unit('cycles/second'));
                    
                otherwise
                    warning(['Unit known as "' x '" is not in the conversion table! Empty value will be applied as unit.']);
                    y=units2convFac(unit(''));
            end % switch
        end % function
