...
h = uibutton(...
    ...
    'ButtonPushedFcn',@(~,~) app.importFromMatlab(app,0,'workspace'),...
...
h = uilistbox(...
    ...
    'ValueChangedFcn',@(hObject,~) Listbox_Callback(hObject,[],app),...
    'MultiSelect','on',...
    'Items',{},...
    'Value',{},...
    'Tag','listbox1');
...
h = uicontrol(...
    ...
    'BackgroundColor',[1 1 1],...
    'Callback',@(hObject,eventdata)kviewOLD('CustomMenu_popupmenu_Callback',hObject,eventdata,app.GUI,1),...
...
app.GUI.(get(h,'Tag')) = h;
h = uix.Empty('Parent',app.GUI.HBox2,'Tag','Empty6eee');
app.GUI.(get(h,'Tag')) = h;
% h = uidropdown(...
%     'Parent',app.GUI.HBox2,...
%     'BackgroundColor',[1 1 1],...
%     'ValueChangedFcn',@(hObject,eventdata)kviewOLD('CustomMenu_popupmenu_Callback',hObject,eventdata,app.GUI,2),...
%     'Text',{  '-- None --'; 'Offset and Gain'; 'Custom Buttons 1'; 'Custom Buttons 2'; 'Custom Buttons 3' },...
%     'Value','-- None --',...
...
function Listbox_Callback(hObject, ~, app)

ListboxContent = hObject.Items;
DynamicCheckValue = app.GUI.DynamicPlotCheck.Value;
